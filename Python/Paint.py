...
class Paint(object):
    ...
    def paint(self,event):
        ...
        if self.odd_x and self.old_y:
            self.c.create_line(self.odd_x,self.old_y,event.x,event.y,width=self.line_width,fill=paint_color,capstyle=ROUND,smooth=TRUE,splinesteps=36)
            self.odd_x=event.x
            self.odd_y=event.y
